...
object linkResolver {
   ...
   private def internalLink(template : DocTemplateEntity, classpath : Classpath) : Option[Link] = {
      Some(Link(settings.outdir.value + File.separator + classpath.packageFilePath, template.name + settings.outputFormat))
   }
   ...
   private def externalLink(template : TemplateEntity, classpath : Classpath) : Option[Link] = {
      ...
      externalApi match {
         case Some(extApiLink) => Some(Link(extApiLink + classpath.packageFilePath, template.name + settings.outputFormat))
         case None => None
      }
   }
...
case class Link(private val absolute : String, filename : String) {
