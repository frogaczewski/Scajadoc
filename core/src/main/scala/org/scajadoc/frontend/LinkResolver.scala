...
object linkResolver {
   ...
   private final val PACKAGE_LIST_FILE = "package-list"
   ...
   def getLinksToExternalDependencies() = {
      def getPackageList(link : String) : Source = Source.fromURL(new URL(link + PACKAGE_LIST_FILE))
   ...
   private def internalLink(template : DocTemplateEntity, classpath : Classpath) : Option[Link] = {
      Some(Link(settings.outdir.value + template.name + settings.outputFormat))
   }
   ...
   private def externalLink(template : TemplateEntity, classpath : Classpath) : Option[Link] = {
      ...
      externalApi match {
         case Some(extApiLink) => Some(Link(extApiLink + classpath.canonicalFileClasspath + settings.outputFormat))
         case None => None
      }
   }
...
case class Link(val absolute : String) {
   def relative(from : String) : String = ""
   def absoluteLink() : String = absolute
}